...
const MOBILE_NAV_ITEMS: MobileNavItem[] = [
  {
    id: 'orchestra',
    label: 'Hivemind',
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/><line x1="12" y1="2" x2="12" y2="6"/><line x1="12" y1="18" x2="12" y2="22"/><line x1="2" y1="12" x2="6" y2="12"/><line x1="18" y1="12" x2="22" y2="12"/></svg>,
  },
  {
    id: 'activity',
    label: 'Log',
  ...
  },
  {
    id: 'plan',
    label: 'Plan',
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>,
  },
  {
    id: 'code',
    label: 'Code',
    icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>,
  },
  {
    id: 'changes',
    label: 'Diff',
...
const MobileTabNav = React.memo(function MobileTabNav({
  ...
  // Use a DOM ref to read the actual input value on submit.
  ...
  const handleSubmit = (): void => {
    ...
    const val = (inputRef.current?.value ?? message).trim();
    if (val && !sending) {
      onMessageChange('');
    ...
    >
      ...
      {lastTicker && (
        <div className="px-3 pt-1.5 pb-0.5">
          <div className="text-[11px] truncate"
            style={{ color: 'var(--accent-blue)', fontFamily: 'var(--font-mono)', opacity: 0.85 }}>
            {lastTicker}
          </div>
        </div>
      )}

      {/* Tab nav — min-h-[48px] ensures 48px touch targets on all buttons */}
      <div className="flex items-center px-1">
        {MOBILE_NAV_ITEMS.map(item => (
          <button
            ...
            className="flex-1 flex flex-col items-center justify-center py-2.5 min-h-[48px] transition-colors active:opacity-75 active:scale-95"
        ...
        {(projectStatus === 'running' || projectStatus === 'paused') && (
          <div className="flex items-center gap-2 pl-1 ml-1" style={{ borderLeft: '1px solid var(--border-dim)' }}>
        ...
        {projectStatus === 'idle' && activitiesCount > 0 && (
          <button
            ...
            className="p-2 rounded-lg min-h-[40px] min-w-[40px] flex items-center justify-center ml-1 transition-opacity active:opacity-75"
      ...
      <div className="flex items-center gap-1.5 px-2 pt-1" style={{ paddingBottom: 'max(8px, env(safe-area-inset-bottom, 8px))' }}>
        ...
        <button
          ...
          disabled={sending}
          className="p-2.5 rounded-full transition-all flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-[var(--accent-blue)] min-w-[44px] min-h-[44px] flex items-center justify-center active:opacity-75"
          style={{
