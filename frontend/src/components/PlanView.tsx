        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-bold"
              style={{ color: hasFailures ? 'var(--accent-red)' : pct === 100 ? 'var(--accent-green)' : 'var(--accent-blue)', fontFamily: 'var(--font-mono)' }}>
              {pct}%{hasFailures ? ` (${errorCount > 0 ? `${errorCount} failed` : ''}${errorCount > 0 && cancelledCount > 0 ? ', ' : ''}${cancelledCount > 0 ? `${cancelledCount} cancelled` : ''})` : ''}
            </span>
          </div>
              ...
              >
                ...
                <div className={`flex-1 min-w-0 rounded-xl px-3.5 py-2.5 transition-all duration-300`}
                  style={{
                    ...
                    border: isActive ? `1px solid ${accent.color}30` : isError ? '1px solid rgba(245,71,91,0.15)' : isCancelled ? '1px solid rgba(160,160,160,0.1)' : '1px solid transparent',
                    boxShadow: isActive ? `0 0 15px ${accent.glow}` : 'none',
                  }}>
                  <p className={`text-sm leading-relaxed ${isDone ? 'line-through' : ''} ${isCancelled ? 'line-through' : ''}`}
                    style={{
