    ...
    def test_skill_agent_map_values_are_valid_roles(self):
        ...
        for skill, roles in skills_registry.SKILL_AGENT_MAP.items():
            assert isinstance(roles, list), (
                f"Skill '{skill}' value should be a list, got {type(roles)}"
            )
            for role in roles:
