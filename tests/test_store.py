...
def session_mgr(tmp_path):
    db_url = f"sqlite+aiosqlite:///{tmp_path / 'test_platform.db'}"
...
def test_save_and_load_project(session_mgr):
    run(
        session_mgr.save_project(
            "test_proj", user_id=123, name="Test", description="A test", project_dir="/tmp/test"
        )
    )
...
def test_add_and_get_messages(session_mgr):
    run(session_mgr.add_message("proj1", "orchestrator", "assistant", "Hello", 0.01))
    run(session_mgr.add_message("proj1", "developer", "assistant", "Working on it", 0.02))
...
def test_list_projects(session_mgr):
    run(
        session_mgr.save_project(
            "proj1", user_id=1, name="Project 1", description="", project_dir="/tmp/p1"
        )
    )
    run(
        session_mgr.save_project(
            "proj2", user_id=1, name="Project 2", description="", project_dir="/tmp/p2"
        )
    )
